...
# we can now introduce a batch dimension
...
idx = torch.zeros((1,1),dtype=torch.long) # data type is integer 0
print(decode(m.generate(idx,max_new_tokens=100)[0].tolist())) # generate works on the level of batches, so we go to 0th row

# Lets train the model now.
optimizer = torch.optim.AdamW(m.parameters(),lr=1e-3) # we used AdamW with a higher learning rate because it is a small model
batch_size = 32
